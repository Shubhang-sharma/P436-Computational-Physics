...
def root_bisection(f,a,b,e,i=0):
    i+=1
    if f(a)*f(b)>0: #bracket first
        a,b = bracket(f,a,b)[0],bracket(f,a,b)[1]
    ...
    if f(c)*f(a) < 0: #bisect the interval to give a new interval
        ...
        a = c
    return bisect(f,a,b,i) #repeat the whole process for the new interval


def root_regula_falsi(f,a,b,e,i=0):
    i+=1
    if f(a)*f(b)>0: #bracket first
    ...
    if f(c)*f(a) < 0:
        ...
        a = c
    return root_regula_falsi(f,a,b,i)
