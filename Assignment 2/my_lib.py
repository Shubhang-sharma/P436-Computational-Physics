def my_lcg(seed,num,b=1,a=1103515245,c = 12345,m = 32768):
    #a = 1103515245, c = 12345, m = 32768 x0 = 10#
    # Res.append(str(seed) + '  ')
    x = seed
    Y = []
...
def rwalk(seedx,seedy,N):
    import matplotlib.pyplot as plt
    import numpy as np
    r_x = my_lcg(seedx,N,0)
    r_y = my_lcg(seedy,N,0)
    X = [0];Y = [0]
    rm = 0
    for i in range(N):
        x[i] = 2*x[i] - 1
    ...
    # plt.xlim(-coord[0][N-1],coord[0][N-1])
    # plt.ylim(-coord[1][N-1],coord[1][N-1])
